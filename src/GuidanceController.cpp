...
// 전송 by SystemContext
void GuidanceController::startGuidanceTask()
{
...
// 전송 by SystemContext
void GuidanceController::stopGuidanceTask()
{
    if (!running_.exchange(false))
        return; // running_의 값을 false로 바꾸고 이전값 반환(이미 fasle였으면 리턴)
    // if (worker_.joinable()) worker_.join();
}

void GuidanceController::join()
{
    if (worker_.joinable())
        worker_.join();
}
...
// 모드 확인과 변경
void GuidanceController::checkAndUpdateStrategy(missile_state_t missile_now, target_state_t target_now)
{
    Vec3 R = sub3(target_now.r_t, missile_now.r_m);
...
void GuidanceController::setMode(GuidanceMode m)
{
    std::lock_guard<std::mutex> lock(mtx_);
    mode_ = m;
}

void GuidanceController::setFlightStart(TimePoint tp)
{
    flight_time_ = tp;
